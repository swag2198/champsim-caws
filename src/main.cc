...
uint64_t va_to_pa(uint32_t cpu, uint64_t instr_id, uint64_t va, uint64_t unique_vpage, uint8_t is_code)
{
    ...
    map <uint64_t, uint64_t>::iterator pr = page_table.begin(); /* Bug: page_table is int to int map */
    ...
    if (pr == page_table.end()) { // no VA => PA translation found /* Bug: .end will be .end() */
        ...
        if (allocated_pages >= DRAM_PAGES) { // not enough memory
            ...
            for (pr = page_table.begin(); pr != page_table.end(); pr++) { /* Bug: .begin, .end */
            ...
            page_table.erase(pr); /* Bug- spelling here and next line */
